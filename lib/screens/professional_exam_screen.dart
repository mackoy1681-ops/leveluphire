...
class ProfessionalExamScreen extends ConsumerStatefulWidget {
  final String profession;
  final List<Question> questions;
  
  const ProfessionalExamScreen({
    super.key,
  ...
  @override
  ConsumerState<ProfessionalExamScreen> createState() => _ProfessionalExamScreenState();
}

class _ProfessionalExamScreenState extends ConsumerState<ProfessionalExamScreen> {
  ...
  Future<void> _generateSummaryFromGemini() async {
    ...
    final prompt = '''
I am a ${widget.profession} professional. I scored $percentage% on a professional certification exam (${_score} out of ${_selectedQuestions.length} correct).

Provide a detailed, professional summary of my performance. Be encouraging but honest. Mention what this score means for a ${widget.profession}. Give specific areas I should improve on. Be at least 3-4 sentences. Sound like a professional mentor.
''';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final String summary = data['candidates'][0]['content']['parts'][0]['text'];
        setState(() {
          _generatedSummary = summary;
        });
      } else {
        _generatedSummary = 'You scored $percentage% on the ${widget.profession} exam. ${percentage >= 76 ? 'Good work! Continue building your expertise.' : 'Keep studying and try again.'}';
      }
    } catch (e) {
      _generatedSummary = 'You scored $percentage% on the ${widget.profession} exam. ${percentage >= 76 ? 'Good work!' : 'Keep practicing.'}';
    }
  }
  ...
  Future<void> _autoSaveResult() async {
    ...
    await _generateSummaryFromGemini();
    ...
    if (percentage >= 76) {
      ...
      try {
        final existing = await Supabase.instance.client
            ...
            .select()
            .eq('user_id', user.id)
            .eq('test_name', 'Professional Exam')
            .maybeSingle();
        ...
        print('Error saving professional exam result: $e');
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Professional Exam', style: TextStyle(color: Colors.white)),
                      ...
                      Expanded(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'Measure your expertise in ${widget.profession}',
